 ...
 * with added ASR envelope system for expressive control.
 *
...
void AudioSynthBraidsFluted::begin(void)
{
    ...
    setRelease(100.0f);    // 100ms default release
    
    // Default envelope routing: modulate amplitude only
    env_route_timbre_    = false;
    env_route_color_     = false;
    env_route_amplitude_ = true;
    
    // Default envelope depths: full modulation
    env_depth_timbre_    = 1.0f;
    env_depth_color_     = 1.0f;
    env_depth_amplitude_ = 1.0f;

    timbre_effective_    = timbre_base_;
    color_effective_     = color_base_;
    amplitude_effective_ = 0.0f;  // starts silent (envelope at 0)
...
{
    // Trigger release stage (envelope will decay to 0)
    if (env_stage_ != ENV_IDLE) {
...
void AudioSynthBraidsFluted::setAttack(float milliseconds)
{
    if (milliseconds < 0.1f) milliseconds = 0.1f;
    // Increment per sample to go from 0 to 1 in the given time
    // At 44100 Hz: samples = ms * 44.1
    float samples = milliseconds * 44.1f;
    attack_increment_ = 1.0f / samples;
}

void AudioSynthBraidsFluted::setRelease(float milliseconds)
{
    if (milliseconds < 0.1f) milliseconds = 0.1f;
    float samples = milliseconds * 44.1f;
    release_increment_ = -1.0f / samples;  // negative (decay)
}

void AudioSynthBraidsFluted::setEnvelopeRouting(bool routeTimbre, bool routeColor, bool routeAmplitude)
{
    env_route_timbre_    = routeTimbre;
    env_route_color_     = routeColor;
    env_route_amplitude_ = routeAmplitude;
    applyEnvelopeModulation();
}

void AudioSynthBraidsFluted::setTimbreEnvDepth(float depth)
{
    if (depth < 0.0f) depth = 0.0f;
    if (depth > 1.0f) depth = 1.0f;
    env_depth_timbre_ = depth;
    applyEnvelopeModulation();
}

void AudioSynthBraidsFluted::setColorEnvDepth(float depth)
{
    if (depth < 0.0f) depth = 0.0f;
    if (depth > 1.0f) depth = 1.0f;
    env_depth_color_ = depth;
    applyEnvelopeModulation();
}

void AudioSynthBraidsFluted::setAmplitudeEnvDepth(float depth)
{
    if (depth < 0.0f) depth = 0.0f;
    if (depth > 1.0f) depth = 1.0f;
    env_depth_amplitude_ = depth;
    applyEnvelopeModulation();
}

// ---- Internal envelope methods ----

void AudioSynthBraidsFluted::updateEnvelope(void)
{
    switch (env_stage_) {
        case ENV_IDLE:
            // Do nothing
...
void AudioSynthBraidsFluted::applyEnvelopeModulation(void)
{
    // For each parameter: effective = base * (1 - depth + depth * envelope)
    // This way:
    //   - depth=0: effective = base (no modulation)
    //   - depth=1, env=0: effective = 0 (full modulation, envelope at bottom)
    //   - depth=1, env=1: effective = base (full modulation, envelope at top)
    
    if (env_route_timbre_) {
        timbre_effective_ = timbre_base_ * (1.0f - env_depth_timbre_ + env_depth_timbre_ * env_level_);
    } else {
        timbre_effective_ = timbre_base_;
    }
    
    if (env_route_color_) {
        color_effective_ = color_base_ * (1.0f - env_depth_color_ + env_depth_color_ * env_level_);
    } else {
        color_effective_ = color_base_;
    }
    
    if (env_route_amplitude_) {
        amplitude_effective_ = amplitude_base_ * (1.0f - env_depth_amplitude_ + env_depth_amplitude_ * env_level_);
    } else {
        amplitude_effective_ = amplitude_base_;
    }
}
...
void AudioSynthBraidsFluted::recomputeDelays(void)
{
    ...
    // Use effective color (after envelope modulation)
    breath_intensity_ = (uint16_t)(2100.0f - (color_effective_ * 1900.0f));
...
void AudioSynthBraidsFluted::update(void)
{
    ...
    for (int i = 0; i < AUDIO_BLOCK_SAMPLES; i++) {
        ...
        // Recompute effective parameters from base + envelope
        // (This could be optimized to only recompute when envelope changes,
        //  but doing it every sample is simple and not too expensive)
        applyEnvelopeModulation();
        
        // If timbre or color changed due to envelope, update delays/breath
        // (In practice we could cache this and only update when needed,
        //  but for clarity we'll recompute each sample)
        // For efficiency, we only recompute the derived values that changed:
        uint16_t timbre_mapped = (uint16_t)(timbre_effective_ * 63.0f);
        uint32_t jet_delay_q16_local = (bore_delay >> 8) * (48 + timbre_mapped);
        uint32_t bore_delay_q16_local = bore_delay + jet_delay - jet_delay_q16_local;
        // Note: We're reusing the total delay and just adjusting the split.
        // This is a simplification — ideally we'd cache the total and only
        // recompute when frequency changes. For full accuracy:
        breath_int = (uint16_t)(2100.0f - (color_effective_ * 1900.0f));

        // Read bore delay line
        ...
        // Output with envelope-modulated amplitude
        int32_t out = bore_value >> 1;
        if (out > 32767)  out =  32767;
        if (out < -32768) out = -32768;
        out = (out * (int32_t)(amplitude_effective_ * 32767.0f)) >> 15;

        bp[i] = (int16_t)out;

        // Advance excitation pointer
        if (i & 3) {
            excitation_ptr++;
